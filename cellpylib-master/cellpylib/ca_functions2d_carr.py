...
def plot2d_animate(ca, title=''):
    ...
    fig = plt.figure()
    plt.title(title)
    im = plt.imshow(ca[0], animated=True, cmap=cmap, alpha=0.3, interpolation='bilinear')
    #im2_map = mpimg.imread('/home/samyaknn/Documents/FireCellularAutomata/wind+topo.png')
    img = cv2.imread('/home/samyaknn/Documents/FireCellularAutomata/wind+topo-1.tif')
    res = cv2.resize(img, dsize=(300, 300), interpolation=cv2.INTER_CUBIC)
    im2 = plt.imshow(res, alpha=0.9, interpolation='bilinear')
    i = {'index': 0}
    def updatefig(*args):
        i['index'] += 1
        if i['index'] == len(ca):
...
def init_random_fire_map2d(rows, cols, k = 1, dtype = np.float_, spread='random'):
	...
	wind_x_map = np.full((rows, cols), 1)
	wind_x_map = np.array([wind_x_map])
	wind_y_map = np.full((rows, cols), 1)
...
def fire_spread_rule(neighborhood, c, t): 
	...
	for i in range(3):
		for j in range(3):
			if (i == 0 and j == 0):
				...
				if (fire_load_center < fl_threshold):
					if h > 0:
	...
	if neighborhood[1][1][1] == 1:
		neighborhood[0][1][1] -= burn_rate
		if (neighborhood[0][1][1] <= 0):
			neighborhood[0][1][1] = 0
			neighborhood[1][1][1] = 0
	if (neighborhood[1][1][1] == 0 and neighborhood[2][1][1] > state_change_threshold and neighborhood[0][1][1] != 0):
