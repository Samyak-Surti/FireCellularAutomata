    ...
    for t in range(1, timesteps):
    	...
    	for row in range(cell_layer[0][0].size):
    		for col in range(cell_layer[0][0].size):
    			...
    			for a in range(rows):
    				for b in range(cols):
    					mag = math.pow((math.pow(cell_layer[3][a][b], 2) + math.pow(cell_layer[4][a][b], 2)), 1/2)
    					cell_layer[3][a][b] = mag * math.cos(math.atan(cell_layer[4][a][b]/cell_layer[3][a][b]) + math.pi/16)
    					cell_layer[4][a][b] = mag * math.sin(math.atan(cell_layer[4][a][b]/cell_layer[3][a][b]) + math.pi/16)
    			"""
    			#print(n[2][1][1])
    			for i in range(6):
...
def init_random_fire_map2d(rows, cols, k = 1, dtype = np.float_, spread='random'):
	...
	if spread == 'uniform':
		...
		fuel_map = np.array([fuel_map])
	if spread == 'random':
		fuel_map = np.random.uniform(0,1, size = (rows, cols)).astype(dtype)
		fuel_map = np.array([fuel_map])
...
#General rule to change configuration of a center cell for every cell the cellular automata for every timestep.
